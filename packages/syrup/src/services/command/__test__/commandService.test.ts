...
describe('CommandService', () => {
  ...
  it('should fail if command is not registered', async () => {
    await expect(
      commandService.executeCommand('non.existent.command')
  ...
  it('should fire events before and after execution', async () => {
    ...
    try {
      ...
      expect(willExecuteSpy.mock.invocationCallOrder[0]).toBeLessThan(
        handler.mock.invocationCallOrder[0]
      )

      expect(handler.mock.invocationCallOrder[0]).toBeLessThan(
        didExecuteSpy.mock.invocationCallOrder[0]
      )
    } finally {
      dispose()
    }
