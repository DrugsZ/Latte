...
- Rust/WASM native engine 作为未来可选性能层，只能挂在 worker/service/system 后面。
- `art` 作为只读渲染层。
...
graph TD
    ...
    subgraph Native["Future Native Engine"]
        NativeEngine["@latte-js/native or Rust crate<br/>WASM geometry / storage / replay kernels"]
    end

    subgraph Kernel["Data Kernel"]
        Espresso["@latte-js/espresso<br/>SceneGraph / NodeCursor / SoA / Blob Contract"]
    end
    ...
    Barista --> NativeEngine
    Barista --> Espresso
    NativeEngine --> Espresso
    Barista --> Bean
...
- 当前 JS heap/blob 是临时 append-only 实现：释放只做 tombstone，不做 free-list、compaction 或长期存储优化。
- Rust/WASM 可以在未来替换 storage/geometry 热路径，但必须保持 pointer columns、SceneGraph/NodeCursor 和 worker service contract 稳定。
...
- 有稳定性能基线证明 JS 路径成为瓶颈。
- JS fallback 可用。
- native 模块只在 worker 内部被 service/system 调用。
- 主线程、插件和 renderer 不获得 raw native memory 写权限。
...
Native engine is optional and hidden behind worker boundaries.
Art owns rendering.
