...
import { ShadowGroup } from 'Latte/core/selection/activeSelectionWidget'
...
export class ActiveSelection {
  ...
  private _shadowGroup = new ShadowGroup()
  ...
  private get _objects() {
    return this._shadowGroup.getChildren()
  }

  private set _objects(values: DisplayObject[]) {
    this._shadowGroup.removeChild()
    this._shadowGroup.appendChild(...values)
    this._onActiveSelectionChange.fire(this)
  }

  public addSelectElement = (element: DisplayObject) => {
    if (!element) return
    this._shadowGroup.appendChild(element)
  }

  public removeSelectElement(element: DisplayObject) {
    if (!element) return
    this._shadowGroup.removeChild(element)
  }
  ...
  get OBB() {
    return this._shadowGroup.OBB
  }

  public getBounds() {
    return this._shadowGroup.getBounds()
  }

  public getCenter() {
    return this._shadowGroup.getCenter()
  }
  public hasActive() {
  ...
  public hasSelected = (element: DisplayObject) => {
    return this._shadowGroup.getChildren().includes(element)
  }
  ...
  public isActive() {
    return this._shadowGroup.getChildren().length > 0
  }
  ...
  public override getFills() {
    const fills: Paint[] = []
    this._shadowGroup.getChildren().forEach(item => {
  ...
  public override getOBBPoints() {
    return this._shadowGroup.getOBBPoints()
  }
}
